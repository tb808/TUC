...
const wideStrike = (kind: TechniqueKind) => ['hook', 'elbow', 'kick', 'sideKick'].includes(kind);
...
export function defenseAvoids(target: Fighter, technique: Technique) {
  if (target.defenseTime <= 0) return false;
  if ((target.defense === 'slipLeft' || target.defense === 'slipRight') && technique.zone === 'head') return technique.kind === 'punch' || technique.kind === 'uppercut';
  return target.defense === 'pull' && technique.zone === 'head' && ['punch', 'hook', 'uppercut', 'elbow'].includes(technique.kind);
}
export function canStrikeHit(attacker: Fighter, target: Fighter, attack: Attack): boolean {
  return strikeWouldHit(attacker, target, attack) && !defenseAvoids(target, attack.technique);
}
export function impactQuality(attacker: Fighter, target: Fighter, technique: Technique) {
  const d = distance(attacker.position, target.position);
  const ideal = technique.kind === 'elbow' || technique.kind === 'knee' ? .58 : technique.kind === 'hook' || technique.kind === 'uppercut' ? .72 : technique.kind === 'kick' || technique.kind === 'frontKick' || technique.kind === 'sideKick' ? 1.16 : .9;
  const tolerance = technique.kind === 'elbow' || technique.kind === 'knee' ? .42 : .75;
  const range = clamp(1 - Math.abs(d - ideal) / tolerance * .32, .58, 1);
  const speed = Math.hypot(attacker.velocity.x, attacker.velocity.z);
  const planted = clamp(1 - speed * (technique.kind === 'punch' ? .07 : .12), .72, 1);
  const stance = attacker.stanceSwitch > 0 ? .72 : technique.hand ? 1.06 : .96;
  return range * planted * stance;
}
...
export class Combat {
  ...
  constructor(rules: Partial<MatchRules> = {}, readonly training = false, readonly profiles: readonly [FighterProfile, FighterProfile] = [FIGHTERS[0], { ...FIGHTERS[1], stats: STATS }]) { this.fighters = [createFighter(0, profiles[0]), createFighter(1, profiles[1])]; this.rules = { ...RULES, ...rules }; this.remaining = this.rules.roundSeconds; }
  ...
  private tryAction(f: Fighter, action: string, direction?: string): boolean {
    ...
    if (action === 'stance') {
      ...
      this.message(f.id === 0 ? (f.stance === 'orthodox' ? 'ORTHODOXE AUSLAGE' : 'SOUTHPAW-AUSLAGE') : 'GEGNER WECHSELT AUSLAGE'); return true;
    }
    if (['parry', 'check', 'slip-left', 'slip-right', 'pull'].includes(action)) return this.tryDefense(f, action);
    ...
    if (this.grapple) {
      if (this.grapple.mode === 'submission' || this.grapple.mode === 'takedown' || this.grapple.mode === 'standup' || this.grapple.transition) return false;
  ...
  private tryDefense(f: Fighter, action: string) {
    ...
    if (move === 'slipLeft' || move === 'slipRight') {
      ...
      this.recoil[f.id].x += Math.cos(f.heading) * side * 1.1; this.recoil[f.id].z -= Math.sin(f.heading) * side * 1.1;
    } else if (move === 'pull') {
      this.recoil[f.id].x -= Math.sin(f.heading) * 1.25; this.recoil[f.id].z -= Math.cos(f.heading) * 1.25;
    }
    return true;
  }
  private canChain(previous: Technique, next: Technique) {
    if (previous.hand === next.hand && previous.kind !== 'punch') return false;
    if (['sideKick', 'frontKick', 'knee', 'elbow'].includes(previous.kind)) return false;
    if (previous.kind === 'kick') return next.kind === 'punch' && previous.hand !== next.hand;
    if (next.kind === 'sideKick' || next.kind === 'frontKick') return previous.kind === 'punch' && previous.hand === 0;
    return previous.hand !== next.hand || next.kind === 'knee';
  }
    ...
    if (enemy.state !== 'knockedDown' && enemy.state !== 'finished') {
      if (linked && !a.hit && a.elapsed >= t.windup && a.elapsed <= t.windup + t.active) { a.hit = true; this.applyHit(f, enemy, a); }
        ...
        if (defenseAvoids(enemy, t)) {
          enemy.counterWindow = .72; f.cooldown = Math.max(f.cooldown, .18);
          this.message(enemy.defense === 'pull' ? 'PULL COUNTER BEREIT' : 'SLIP · KONTERFENSTER');
  ...
  private applyHit(attacker: Fighter, target: Fighter, a: Attack) {
    ...
    if (!blocked && attacker.counterWindow > 0) { this.message('SAUBERER KONTER'); attacker.counterWindow = 0; }
  ...
  private tryGrapple(f: Fighter, action: string, direction?: string) {
    if (f.attack || f.damage.stamina < 16 || this.grapple?.mode === 'standup') return false;
    const enemy = this.fighters[other(f.id)];
    if (!this.grapple) {
    ...
    if (g.mode === 'clinch' && !g.transition) {
      g.transition = { by: f.id, direction: (direction ?? 'advance') as GroundDirection, elapsed: 0, duration: .8 / f.stats.grappling }; f.damage.stamina -= 10; f.cooldown = .9 / f.stats.grappling; return true;
    }
    return false;
  }
  private tryStand(f: Fighter) {
    ...
    if (g.mode === 'clinch' || g.top === f.id || (g.position === 'guard' && this.fighters[g.top].damage.stamina < f.damage.stamina + 8)) {
      ...
      else { g.mode = 'standup'; g.timer = 0; this.fighters.forEach(x => { x.attack = null; x.state = 'ground'; }); this.message('AUFSTEHEN'); }
      return true;
    }
  ...
  private updateGrapple(dt: number) {
    ...
    let top = this.fighters[g.top], bottom = this.fighters[other(g.top)];
    if (g.mode === 'standup') {
      // Separate while the fighters rise, so returning to footwork has no position jump.
      const d = distance(top.position, bottom.position);
      const ax = Math.sin(top.heading), az = Math.cos(top.heading);
      const travel = Math.min(dt * 1.1, Math.max(0, 1.18 - d) / 2);
      top.position = insideCage({ x: top.position.x - ax * travel, z: top.position.z - az * travel }, this.rules.cageApothem);
      bottom.position = insideCage({ x: bottom.position.x + ax * travel, z: bottom.position.z + az * travel }, this.rules.cageApothem);
      if (g.timer >= .62) { this.release(false); this.message('ZURÜCK IM STAND'); }
      return;
    }
    ...
    if (g.mode === 'takedown' && g.timer >= .78) {
      g.mode = 'ground'; g.timer = 0; this.fighters.forEach(f => { f.state = 'ground'; f.attack = null; }); this.score.grappling[g.top] += 6; this.message('AM BODEN · Guard');
      return;
    }
    if (g.transition) {
      ...
      if (tr.defended) {
        ...
        if (tr.elapsed <= 0) { g.transition = null; this.fighters[tr.by].cooldown = Math.min(this.fighters[tr.by].cooldown, .18); }
      } else {
        tr.elapsed += dt;
        if (!tr.resolved && tr.elapsed >= duration * .62) {
          tr.resolved = true;
          const actor = this.fighters[tr.by], defender = this.fighters[other(tr.by)];
          const defended = defender.guard !== null && defender.damage.stamina * defender.stats.grappling >= actor.damage.stamina * actor.stats.grappling * .55 && defender.damage.stamina > 10;
        ...
        if (!tr.defended && tr.elapsed >= duration) {
          if (g.mode === 'clinch') { g.top = tr.by; this.score.grappling[tr.by] += 2; this.message('CLINCH-KONTROLLE'); }
            ...
            if (tr.flips) { g.top = tr.by; g.side = (-(g.side ?? 1)) as -1 | 1; top = this.fighters[g.top]; bottom = this.fighters[other(g.top)]; this.message('SWEEP · Position gedreht'); }
    ...
    if (g.mode === 'ground' && g.transition?.from && g.transition.to) {
      ...
      if (tr.flips) lateral += (tr.direction === 'left' ? -1 : 1) * Math.sin(p * Math.PI) * .18;
    }
  ...
  private release(spread = true) {
    this.grapple = null;
    this.fighters.forEach(f => { f.state = 'idle'; f.attack = null; f.cooldown = .4; if (spread) f.position = insideCage({ x: f.position.x - Math.sin(f.heading) * .42, z: f.position.z - Math.cos(f.heading) * .42 }, this.rules.cageApothem); });
  }
