...
export interface Fighter { id: FighterId; name: string; position: Vec2; velocity: Vec2; heading: number; state: CombatState; stats: FighterStats; damage: DamageState; attack: Attack | null; guard: 'high' | 'low' | null; stun: number; cooldown: number; knockdowns: number; knockdownTime: number; reaction: number; reactionSide: number; cut: number; swelling: number; unanswered: number; lastHit: number }
...
export type CombatEvent = { type: 'hit'; attacker: FighterId; target: FighterId; technique: string; zone: Zone; strength: number; blocked: boolean; position: Vec2 } | { type: 'message'; text: string } | { type: 'bell' } | { type: 'end'; result: MatchResult };
